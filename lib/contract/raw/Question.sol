...
contract Question {
    ...
    constructor(
        ...
        uint256 _startVoteAt,
        uint256 _voteDuration
    ) {
        timestampData = TimestampData(
            block.timestamp,
            block.timestamp + _startBetAt,
            block.timestamp + _startVoteAt,
            block.timestamp + _startBetAt + _betDuration + _betDuration,
            block.timestamp + _startVoteAt + _voteDuration
        );
        ...
        )
    {
        return (
            ...
            sumBets,
            totalBetAmount,
            timestampData.startBetAt,
            timestampData.endBetAt,
            timestampData.startVoteAt,
            timestampData.endVoteAt
        );
    }
    ...
    function getChoiceVoteData(string memory choiceName)
        public
        view
        returns (uint256)
    {
    ...
    function getChoiceBetData(string memory choiceName)
        public
        view
        returns (uint256, uint256)
    {
        ...
        require(
            ...
            "You can access choice bet data after ending voting period."
        );
        return (
            choiceDataMapping[choiceName].bettor.length,
            choiceDataMapping[choiceName].betAmounts
        );
    }
    ...
    function vote(string memory choiceName) public {
        require(
            ...
            "There isn't in voting period."
        );
        require(
            !userDataMapping[msg.sender].isBet &&
                !userDataMapping[msg.sender].isVote,
            "You already bet or vote therefore you can't vote again."
        );
        require(_findChoice(choiceName), "This choice isn't exist!");
        userDataMapping[msg.sender] = UserData(
            0,
            false,
            true,
            0,
    ...
    function bet(string memory choiceName, uint256 amount) public {
        require(
            block.timestamp >= timestampData.startBetAt &&
                block.timestamp <= timestampData.endBetAt,
            "There isn't in betting period."
        );
        require(_findChoice(choiceName), "This choice isn't exist!");
        require(
            platformToken.balanceOf(msg.sender) >= 1e19,
            "You must own at least 10 Btr first to start betting"
        );
        require(
            !userDataMapping[msg.sender].isBet &&
                !userDataMapping[msg.sender].isVote,
            "You already bet or vote therefore you can't bet again."
        );
        require(
            platformToken.balanceOf(msg.sender) >= amount,
            "Insufficent balance."
        );
    ...
    function forceCloseQuestion(questionMode mode) public {
        require(
            msg.sender == questionCreator,
            "You can't close voting or bidding without permission."
        );
        if (mode == questionMode.BET) {
            require(
                block.timestamp >= timestampData.startBetAt &&
                    block.timestamp <= timestampData.endBetAt,
                "The question isn't in betting time"
            );
            ...
            require(
                block.timestamp >= timestampData.startVoteAt &&
                    block.timestamp <= timestampData.endVoteAt,
                "The question isn't in voting time"
            );
            timestampData.endVoteAt = block.timestamp;
        }
    }
    ...
    function _getWinner() private view returns (bool[16] memory, uint256) {
        ...
        for (uint256 i = 0; i < choices.length; i++) {
            uint256 choiceVote = choiceDataMapping[choices[i]].vote;
    ...
    function closeQuestion() public {
        require(
            msg.sender == questionCreator,
            "You don't have a permission to close a question."
        );
        require(
            block.timestamp > timestampData.endVoteAt &&
                block.timestamp > timestampData.endBetAt,
            "The question doesn't finalize you can't close"
        );
        ...
        for (uint256 _idx = 0; _idx < choices.length; _idx++) {
            if (winner[_idx]) {
                calculateSumWinner += choiceDataMapping[choices[_idx]]
                    .betAmounts;
            }
        }
        ...
        for (uint256 _idx = 0; _idx < choices.length; _idx++) {
            if (winner[_idx]) {
                address[] memory _bettors = choiceDataMapping[choices[_idx]]
                    .bettor;
                for (uint256 i = 0; i < _bettors.length; i++) {
                    uint256 eachReward = (totalWinnerReward *
                        userDataMapping[_bettors[i]].betAmount) /
                        calculateSumWinner;
    ...
    function harvestReward() public {
        platformToken.transfer(
            msg.sender,
            userDataMapping[msg.sender].userReward
        );
    }
        ...
        )
    {
        ...
        return (
            _userData.userReward,
            _userData.isBet,
            _userData.isVote,
            _userData.betAmount,
            _userData.choiceBet,
            _userData.choiceVote
        );
    }
}
